...
public class HelixEnemy : MonoBehaviour
{
    public float speed;
    ...
    {
        isShooting = true;
        LeanTween.move(gameObject, transform.position-transform.up * .7f, .3f).setEaseInCubic();
        coolDownTimerShooting = Time.time + Random.Range(-0.6f, .6f);
        GameObject p = Instantiate(projectile, transform.position, transform.rotation);
        p.GetComponent<HelixProjectile>().setSpeed(projectileSpeed);
        GameObject p2 = Instantiate(projectile, transform.position, transform.rotation);
        p2.GetComponent<HelixProjectile>().setSpeed(projectileSpeed);

        p2.GetComponent<HelixProjectile>().magnitude *= -1;
